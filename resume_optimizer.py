...
class SimplifiedResumeOptimizer:
    ...
    def extract_existing_keywords(self, latex_content: str, target_keywords: List[str]) -> Set[str]:
        ...
        logger.info(f"Found {len(existing)} existing keywords: {list(existing)}")
        return existing
...
Return only the indices (numbers) of CHANGEABLE bullets, comma-separated."""
    ...
    def count_words_strict(self, text: str) -> int:
        ...
        # Count actual words
        words = [w for w in clean_text.split() if w.strip() and not w.isdigit()]
        return len(words)
...
Return ONLY the rewritten bullet.""")
        ...
        try:
            ...
            if new_word_count < min_words or new_word_count > max_words:
                logger.warning(f"Word count violation: {new_word_count} not in range [{min_words}, {max_words}]")
                return component.original_content
    ...
    def optimize_resume(self, latex_content: str, keywords_or_jd: str, is_jd: bool = False, strict: bool = False) -> str:
        ...
        existing_keywords = self.extract_existing_keywords(latex_content, target_keywords)
        keyword_usage_count = {}
        ...
        keywords_to_add = [kw for kw in target_keywords if keyword_usage_count.get(kw, 0) < 2]
        ...
        changeable_components = self.detect_changeable_components(latex_content)
        ...
        for i, component in enumerate(changeable_components):
            ...
            available_keywords = [kw for kw in keywords_to_add if keyword_usage_count.get(kw, 0) < 2]
            ...
            optimized_content = self.optimize_component_strict(component, component_keywords)
            ...
            if optimized_content != component.original_content:
                ...
                for keyword in component_keywords:
                    if keyword.lower() in optimized_content.lower() and keyword.lower() not in component.original_content.lower():
                        keyword_usage_count[keyword] = keyword_usage_count.get(keyword, 0) + 1
                        logger.info(f"Keyword '{keyword}' now used {keyword_usage_count[keyword]} times")
                ...
                logger.info(f"Words: {self.count_words_strict(component.original_content)} → {self.count_words_strict(optimized_content)}")
...
def main():
    parser = argparse.ArgumentParser(description="Simplified LaTeX resume optimizer")
